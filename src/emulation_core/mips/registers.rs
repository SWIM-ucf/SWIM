...
/// Collection of registers used by the datapath.
...
/// Iterator that is used to view each register in the register file. This
/// contains a copy of all the registers and their values, and a [`RegisterTypeIter`],
/// as generated by [`strum::IntoEnumIterator`]. In other iterator implementations,
/// the internal state might be data like a [`RegisterType`]. However, since we can't
/// normally just "add 1" to get to the next register, we use an internal iterator
/// that can track the progression of one [`RegisterType`] to the next.
pub struct RegistersIter {
    registers: Registers,
    register_iter: RegisterTypeIter,
}

/// This implementation of the [`Iterator`] trait essentially wraps the existing
/// [`RegisterTypeIter`] so that the register type can be paired with register data.
impl Iterator for RegistersIter {
    type Item = (RegisterType, u64);
...
/// an [`Iterator`]. In this case, this is an instance of [`RegistersIter`] with all the
/// data in the registers and a new [`RegisterTypeIter`].
impl IntoIterator for Registers {
    type Item = (RegisterType, u64);
    type IntoIter = RegistersIter;

    /// Consumes the [`Registers`] struct to create a new [`RegistersIter`] that can
    /// be iterated over.
    fn into_iter(self) -> Self::IntoIter {
        RegistersIter {
            registers: self,
            register_iter: RegisterType::iter(),
        }
    }
}
