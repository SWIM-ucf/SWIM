...
fn read_instruction(mut instruction: Instruction) -> Instruction{
    ...
    match &*instruction.tokens[0]
    {
        "add" => {
            ...
            read_operands(&mut instruction, vec![RegisterGp, RegisterGp, RegisterGp], vec![3, 1, 2]);
        ...
        "addi" => {
            ...
            read_operands(&mut instruction, vec![RegisterGp, RegisterGp, Immediate], vec![2, 1, 3]);
...
fn read_operands(instruction: &mut Instruction, expected_operands: Vec<OperandType>, concat_order: Vec<i32>) -> &mut Instruction {
        ...
        match expected_operands[i]{
            ...
            Immediate => {
                ...
                match immediate_results.1{
                    None => {string_representations.push(immediate_results.0);}
                    Some(error) => {instruction.errors.push(error)}
                }
            },
            MemoryAddress => {
    ...
    if instruction.errors.is_empty() {
        for element in concat_order {
            instruction.binary_representation.push_str(string_representations.get(element as usize - 1).unwrap());
        }
    }
...
pub(crate) fn read_memory_address(orig_string: &str, token_number: i32) -> (String, String, Option<Vec<Error>>){
    ...
    if close_index.is_none() || open_index.is_none() || close_index < open_index {
        return ("".to_string(), "".to_string(), Some(vec![Error {
    ...
    return(immediate_results.0, register_results.0.parse().unwrap(), None);
}
